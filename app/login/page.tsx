...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast.error(error.message); // ✅ NEW POPUP
      setLoading(false);
    } else {
      toast.success("Welcome back!"); // ✅ NEW POPUP
        ...
        <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="space-y-1.5">
                ...
                <div className="relative group">
                    ...
                    <input type={showPassword ? "text" : "password"} required placeholder="••••••••" className="w-full bg-black/50 border border-gray-800 rounded-xl py-3 pl-12 pr-12 text-white focus:border-purple-500 outline-none transition-all" onChange={(e) => setPassword(e.target.value)} />
                ...
                <div className="text-right">
                    <Link href="/forgot-password" className="text-xs text-purple-400 hover:text-purple-300 font-medium transition-colors">Forgot Password?</Link>
                </div>
            </div>

            <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-purple-900/30 transition-all flex items-center justify-center gap-2 mt-4">
                {loading ? <Loader2 className="animate-spin"/> : <>Login to Dashboard <ArrowRight size={20}/></>}
            </button>
        </form>

        <p className="text-center text-gray-500 text-sm mt-8">Don't have an account? <Link href="/register" className="text-white font-bold hover:underline">Join Now</Link></p>
      </div>
    </div>
  );
}
