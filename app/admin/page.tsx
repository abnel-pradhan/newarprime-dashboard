...
import { Check, X, LogOut, Wallet, UserPlus, CreditCard } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const fetchData = async () => {
    ...
    // 1. Fetch User Approvals (Incoming Money)
    ...
    // 2. Fetch Withdrawal Requests (Outgoing Money)
    const { data: withData } = await supabase
      .from('withdrawals')
      .select(`*, profiles:user_id (full_name, email, phone_number, bank_account_no, bank_ifsc)`)
      .eq('status', 'pending')
      .order('created_at', { ascending: false });
  ...
  const handleApproveUser = async (paymentId: string, userId: string, amount: number, referrerId: string | null) => {
    if (!window.confirm("Approve this user and distribute commission?")) return;
    ...
    // 3. Distribute Commission (60%)
    if (referrerId) {
        const commission = amount * 0.60;
        const { data: referrer } = await supabase.from('profiles').select('wallet_balance').eq('id', referrerId).single();
        if (referrer) {
            const newBalance = (referrer.wallet_balance || 0) + commission;
  ...
  const handleRejectUser = async (paymentId: string) => {
  ...
  const handlePayWithdrawal = async (withdrawalId: string, userId: string, amount: number) => {
      if (!window.confirm(`Mark ₹${amount} as SENT to user?`)) return;
    ...
    <div className="min-h-screen bg-black text-white p-6 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            {/* SECTION 1: WITHDRAWAL REQUESTS (URGENT) */}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-800">
                            {withdrawals.length === 0 ? <tr><td colSpan={4} className="p-8 text-center text-gray-500">No withdrawal requests.</td></tr> : withdrawals.map(w => (
                                <tr key={w.id} className="hover:bg-gray-800/30">
                                    ...
                                    <td className="p-4">
                                        <div className="text-blue-400 font-mono">{w.payout_upi}</div>
                                        {w.profiles?.bank_account_no && (
                                            <div className="text-[10px] text-gray-500 mt-1">
                                                Bank: {w.profiles.bank_account_no} <br/> IFSC: {w.profiles.bank_ifsc}
                                            </div>
                                        )}
                                    </td>
            ...
            <div className="bg-neutral-900 border border-gray-800 rounded-2xl overflow-hidden flex flex-col h-fit">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-black/50 text-gray-400 uppercase text-xs">
                            <tr><th className="p-4">User</th><th className="p-4">Tx ID</th><th className="p-4">Referrer</th><th className="p-4 text-right">Action</th></tr>
                        </thead>
                        <tbody className="divide-y divide-gray-800">
                            {payments.length === 0 ? <tr><td colSpan={4} className="p-8 text-center text-gray-500">No pending approvals.</td></tr> : payments.map(p => (
                                <tr key={p.id} className="hover:bg-gray-800/30">
                                    <td className="p-4">
                                        <div className="font-bold text-white">{p.profiles?.full_name}</div>
                                        <div className="text-xs text-green-400">₹{p.amount} Paid</div>
                                    </td>
                                    <td className="p-4 font-mono text-yellow-500 text-xs">{p.transaction_id}</td>
                                    <td className="p-4 text-xs text-gray-400">{p.profiles?.referred_by || "Direct"}</td>
                                    <td className="p-4 flex justify-end gap-2">
                                        <button onClick={() => handleApproveUser(p.id, p.user_id, p.amount, p.profiles?.referred_by)} className="p-2 bg-green-600 hover:bg-green-500 rounded text-white"><Check size={16}/></button>
                                        <button onClick={() => handleRejectUser(p.id)} className="p-2 bg-red-600 hover:bg-red-500 rounded text-white"><X size={16}/></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
